...
from sqlalchemy import select, update, insert, Column, Integer, ForeignKey, String, func
...
class ReactionRepository(GenericRepository):
    ...
    async def get_by_slack_id_and_date(
        self,
        slack_id: str,
        year: int,
        month: int
    ...
    async def update(self, reaction: Reaction):
        q = update(
            ...
            self.model.id == reaction.id
        ).values(
            reaction.entity_to_data()
        )
        async with async_session_manager() as session:
            await session.execute(q)

    async def get_monthly_reactions_by_to_user_id(
        self,
        to_user_id: int,
        year: int,
        month: int
    ...
    async def count_special_emoji_by_date_and_from_user(
        self,
        from_user_id: int,
        year: int,
        month: int
        ...
        q = select(
            ...
            self.model.emoji == settings.config.SPECIAL_EMOJI,
            self.model.year == year,
            self.model.month == month,
            self.model.from_user_id == from_user_id,
        )
