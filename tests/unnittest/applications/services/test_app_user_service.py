...
class TestAppUserService:
    def setup_class(self):
        ...
        self.service = UserAppService

    def teardown_class(self):
        truncate_tables(["users", "reactions"])

    @pytest.mark.asyncio
    async def test_get_user(self, db, anyio_backend):
        user: User = await self.service.get_user(slack_id=self.user.slack_id)
    ...
    async def test_update_user(self, db, anyio_backend):
        ...
        user: User = await self.service.get_user(slack_id=slack_id)
